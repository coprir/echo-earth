      ...
      {place && (
        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] uppercase tracking-[0.3em]" style={{ color: `hsl(${categoryById(place.category).hue}, 75%, 70%)` }}>
                {categoryById(place.category).label}
                {place.isGem && <span className="ml-2 text-yellow-200/90">✦ hidden gem</span>}
              </p>
          ...
          <div className="mt-4 flex items-center gap-4 text-sm" style={{ color: "var(--ee-text)" }}>
            ...
            {place.openNow != null && (
              <span style={{ color: place.openNow ? "hsl(140, 70%, 65%)" : "hsl(0, 70%, 70%)" }}>{place.openNow ? "open now" : "closed"}</span>
            )}
          </div>
          ...
          {place.source === "echo" && (
            <p className="mt-3 text-[10px]" style={{ color: "var(--ee-text-dim)" }}>
              echo-grown demo place — add a Google Maps key to surface the real city
            </p>
          )}
        </motion.aside>
      )}
